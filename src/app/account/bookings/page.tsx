            ...
            <main className="flex-1 container py-12">
                ...
                <div className="space-y-6">
                    {bookings?.map((booking: any) => (
                        <div key={booking.id} className="p-6 rounded-2xl liquid-glass flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-2xl transition-all">
                            <div>
                                ...
                                <p className="text-sm mt-3 flex gap-4">
                                    <span className="liquid-glass px-2 py-0.5 rounded text-[10px] font-bold">IN: {new Date(booking.check_in_date).toLocaleDateString()}</span>
                                    <span className="liquid-glass px-2 py-0.5 rounded text-[10px] font-bold">OUT: {new Date(booking.check_out_date).toLocaleDateString()}</span>
                                </p>
                            </div>
                    ...
                    {(!bookings || bookings.length === 0) && (
                        <div className="py-20 text-center border-dashed border-2 border-white/10 rounded-2xl liquid-glass">
